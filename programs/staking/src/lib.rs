...
use anchor_spl::{token::{self, CloseAccount, Mint, Token, SetAuthority, MintTo, TokenAccount, Transfer}, associated_token::AssociatedToken};
use spl_token::instruction::AuthorityType;
use solana_program::pubkey;

declare_id!("HeQTtHWw83MhXGQXgWdQdFKbwhRuNDyRcTo648WsXHnT");
...
pub mod staking {
    ...
    pub fn init_mint_authority(_ctx: Context<AuthorityInit>) -> Result<()> {
        Ok(())
    }
    // Allow user to stake a single NFT
    pub fn stake(ctx: Context<Stake>, staking_period: u16, is_one_of_one: bool) -> Result<()> {

        // Check that the staking period is valid
        if staking_period > 2 {
            //return Err(ErrorCode::InvalidStakingPeriod.into())
            return err!(ErrorCode::InvalidStakingPeriod)
        }
    ...
    pub fn collect_full(ctx: Context<CollectFull>) -> Result<()> {
    ...
    // Create PDA that will be used as the Mint Authority for the Reward Token.
    pub fn init_summary(_ctx: Context<SummaryInit>) -> Result<()> {
        Ok(())
    }
}
...
pub struct SummaryInit<'info> {
    ...
    #[account(
        init,
        seeds = [b"summary2".as_ref()],
        bump,
        payer = owner,
        space = SummaryAccount::LEN,
    )]
    pub summary_account: Account<'info, SummaryAccount>,
    pub system_program: Program<'info, System>,
    pub rent: Sysvar<'info, Rent>,
}
...
pub struct Collect<'info> {
    ...
    #[account(
        ...
        associated_token::authority = staking_token_owner
    )]
    pub owner_staking_token_account: Account<'info, TokenAccount>,
    #[account(
        mut,
